...
def depends():
    return [
        'cfg:gate.Measure.${qubit}.params.bias',
        #('templates/spec.py', f"run/spec/$qubit.py", {'qubit': '$qubit'})
    ]
...
def analyze(report, history):
    ...
    report.parameters = {'gate.Measure.${qubit}.params.amp': 1}
...
def oracle(report, history=None, system_state=None):
    """
